...
causes the expressions ⟨exp1⟩ through ⟨expk⟩ to be evaluated in se- quence and the value of the final
...
#|
We can also create objects that handle deposits as well as with- drawals, and thus we can
...
#|
Exercise 3.4: Modify the make-account procedure of Ex- ercise 3.3 by adding another local state variable so that, if an account
...
#|
It is tempting to conclude this discussion by saying that, by intro- ducing assignment and the technique of hiding state in local
...
#|
Exercise 3.5: Monte Carlo integration is a method of esti- mating definite integrals by means of Monte Carlo simula- tion. Consider
computing the area of a region of space de- scribed by a predicate P(x,y) that is true for points (x,y) in the region and false for
...
con- tains the circle above. e desired integral is the area of that portion of the rectangle that lies in the region. We can
...
x1, x2, y1, and y2 for the rectangle, and the number of trials to perform in order to produce the esti- mate. Your procedure should
...
You will find it useful to have a procedure that returns a number chosen at random from a given range. e follow- ing
random-in-range procedure implements this in terms of the random procedure used in Section 1.2.6, which re-turns a nonnegative
...
Design a new rand procedure that is called with an ar- gument that is either the symbol generate or the symbol reset and behaves
as follows: (rand 'generate) produces a new random number; ((rand 'reset) ⟨new-value ⟩) re- sets the internal state variable to the
designated ⟨new-value ⟩. Thus, by resetting the state, one can generate repeatable se- quences. These are very handy to have when
